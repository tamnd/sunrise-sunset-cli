...
// HTTP behaviour is covered in sunrise-sunset_test.go.

func TestDomainInfo(t *testing.T) {
	info := Domain{}.Info()
	if info.Scheme != "sunrise-sunset" {
		t.Errorf("Scheme = %q, want sunrise-sunset", info.Scheme)
	}
...
func TestHostWiring(t *testing.T) {
	...
	if want := "sunrise-sunset://page/wiki/Go"; u.String() != want {
	...
	got, err := h.ResolveOn("sunrise-sunset", "about")
	if err != nil || got.String() != "sunrise-sunset://page/about" {
		t.Errorf("ResolveOn = (%q, %v), want sunrise-sunset://page/about", got.String(), err)
	}
}
