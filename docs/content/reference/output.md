...
describes what users get. Pick a format with `-o`, or let sunrise-sunset choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sunrise-sunset <command> -o table   # aligned columns for reading
sunrise-sunset <command> -o jsonl   # one JSON object per line, for piping
sunrise-sunset <command> -o json    # a single JSON array
sunrise-sunset <command> -o csv     # spreadsheet friendly
sunrise-sunset <command> -o tsv     # tab-separated
sunrise-sunset <command> -o url     # just the URL column
sunrise-sunset <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sunrise-sunset <command> --fields id,title,url
```
...
```bash
sunrise-sunset <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sunrise-sunset <command>            # a table, because this is a terminal
sunrise-sunset <command> | wc -l    # JSONL, because this is a pipe
```
